...
RS
  ...
  if command -v pdflatex >/dev/null 2>&1; then
    pdflatex -interaction=nonstopmode -halt-on-error -output-directory /tmp /tmp/tikz_ok.tex > /tmp/tikz_ok_build.log 2>&1 || {
      echo 'pdflatex failed; showing log tail:'
      tail -n 80 /tmp/tikz_ok_build.log
      exit 1
    }
