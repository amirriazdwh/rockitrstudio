...
test_dataset_size <- function(num_rows, test_number) {
  ...
  result <- tryCatch({
    ...
    if (exists("loaded_data")) {
      ...
      stop("Parallel read failed: 'loaded_data' not created.")
    }
    ...
    list(
      ...
      read_time = read_time,
      read_rate = if (exists("loaded_data")) nrow(loaded_data) / read_time else NA,
      memory_used_mb = total_memory_used,
      agg_time = agg_time,
      filter_time = filter_time,
      categories = if (exists("agg_result")) nrow(agg_result) else NA,
      filtered_rows = if (exists("filtered")) nrow(filtered) else NA
    )
